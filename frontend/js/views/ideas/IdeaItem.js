...
class IdeaItem extends Component {
  ...
  render() {
    ...
    const commentsMark =
      typeof idea.comments !== 'undefined' &&
      idea.comments != null &&
    ...
    const addCommentMark = (
      <div className="row">
        <div className="col-xs-2 col-sm-2 col-md-2 col-lg-2">
          <div className="avatar-container">
            <span
              ...
              className={
                idea.anonymousMode
                  ? 'identity-icon-anonymous'
                  : 'identity-icon-identified'
              }
            />
          </div>
        </div>
        <div className="col-xs-1 col-sm-1 col-md-1 col-lg-1 comment-attachments">
          <div className="avatar-container">
            <span
              onClick={() => addCommentAttachments(this.commentInput.value)}
              className="attachment-clip-enabled"
            />
          </div>
        </div>
        <div className="col-xs-9 col-sm-9 col-md-9 col-lg-9 comment">
          <input
            ...
            placeholder={I18n.t('ideas.item.addAComment')}
            ref={el => {
    ...
    const showCommentError =
      typeof this.props.commentsErrorMessage !== 'undefined';

    return (
      <div className="idea-item-container shadow">
        <div className="body-container" onClick={() => view()}>
          <div className="label-sm-gray m-b-space">{idea.stage}</div>
          <div className="label-lg-base m-b-space">{idea.title}</div>
          <div className="label-sm-gray">{idea.description}</div>
            ...
            <div className="tag-wrapper">
              {idea.tags.map((tag, index) =>
                tag === idea.category ? (
                  <div
                    key={index.toString()}
                    className="label-sm-base card-tag main-tag"
                  >
                    {tag}
                  </div>
                ) : (
                    <div
                      key={index.toString()}
                      className="label-sm-base card-tag"
                    >
                      {tag}
                    </div>
                  )
          ...
          <div className="right-buttons-container">
            <CircleIconButton
              type={liked ? 'already_like' : 'like'}
              onClick={e => this.handleToggleVote(e)}
            />
            {editable && (
        ...
        <div className="footer-container">
          <div className="footer-item label-sm-gray">
            {votes.toString()} {I18n.t('ideas.item.votes')}
          </div>
          <div className="footer-item label-sm-gray">
            {idea.comments === undefined || idea.comments == null
              ? ''
              : idea.comments.length.toString()}{' '}
            {I18n.t('ideas.item.comments')}
          </div>
          <div className="footer-item label-sm-gray">
            {I18n.t('ideas.item.cost')} {idea.expectedCostInCents}
          </div>
          <div className="footer-item label-sm-gray">
            {I18n.t('ideas.item.time')} {idea.expectedTtm}{' '}
            {I18n.t('ideas.item.months')}
          </div>
