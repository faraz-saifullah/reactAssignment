...
class Users extends Component {
  ...
  render() {
    const { usersArr, role, dispatch, usersErrorMessage } = this.props;
    ...
    function checkboxHandler(cell, row) {
      return <input type="checkbox" checked={cell === 'true'} disabled />;
    }
        ...
        <div className="container">
          <div className="text-center">
            {usersErrorMessage !== 'undefined' && (
              <h3 className="red">{usersErrorMessage}</h3>
            )}
          </div>
          ...
          <BootstrapTable
            ...
            keyField="id"
            options={
              role === 'ROLE_ADMIN' ? { afterDeleteRow: this.handleDelete } : {}
            }
            ...
            pagination
          >
            <TableHeaderColumn hidden hiddenOnInsert autovalue dataField="id">
              Username
            </TableHeaderColumn>
            ...
            <TableHeaderColumn
              dataField="ROLE_ADMIN"
              dataFormat={checkboxHandler}
              editable={
                role === 'ROLE_ADMIN'
                  ? {
                      readonly: role !== 'ROLE_ADMIN',
                      type: 'checkbox',
                      options: { values: role === 'true:false' },
                    }
                  : false
              }
            >
          ...
          <span>
            Double click any field to edit it. Fields marked with a * can only
            be edited by admins.
          </span>
        </div>
      );
    ...
    return (
      <div className="text-center">
        ...
        <div>
          Loading data... If you see this for more than a few seconds, your
          session has expired/invalidated and you should logout then log back
          in.
        </div>
      </div>
    );
  ...
  handleEdit(rowBeforeObject, updatedValueKey, updatedValue) {
    ...
    const updatedUser = {
      ...
      enabled:
        typeof rowBeforeObject.enabledString !== 'undefined' &&
        rowBeforeObject.enabledString === 'true',
      admin:
        typeof rowBeforeObject.ROLE_ADMIN !== 'undefined' &&
        rowBeforeObject.ROLE_ADMIN === 'true',
      userManager:
        typeof rowBeforeObject.ROLE_USER_MANAGER !== 'undefined' &&
        rowBeforeObject.ROLE_USER_MANAGER === 'true',
    };
