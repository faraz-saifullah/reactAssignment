...
export function areAllAttachmentsUploaded(idea) {
  ...
  let result = true;
  if (idea.files != null) {
    const pendingUpload = idea.files.filter(
      file =>
        file.persistenceId != null &&
...
function getFilesToRemove(oldFiles, files) {
  const gotOldFiles = oldFiles == null ? [] : oldFiles;
  return getFilesToAdd(files, gotOldFiles);
}
...
export const REMOVE_REMOTE_FILE_ON_NEW_COMMENT =
...
export function uploadFile(ideaId, htmlFormFile) {
  ...
  return dispatch => {
    ...
    return fetchWithAuth(`${API_BASE_URI}/ideas/attach`, config)
...
export function uploadFileOnNewComment(ideaId, htmlFormFile) {
  ...
  return dispatch => {
    ...
    return fetchWithAuth(`${API_BASE_URI}/ideas/comment/attach`, config)
      .then(response => response.json().then(body => ({ body, response })))
      .then(({ body, response }) => {
        if (!response.ok) {
          dispatch(
            updateFileOnNewCommentError(`Failed to upload file. ${body.error}`)
          );
          return Promise.reject('Failed to upload file');
        }
        dispatch(
          uploadFileOnNewCommentSuccess(dispatch, body, ideaId, htmlFormFile)
        );
        return true;
      })
      .catch(err => {
...
export function uploadFileContent(ideaId, persistenceId, htmlFormFile) {
  ...
  if (token && token !== 'undefined') {
    ...
    formData.append(
      'file',
      new Blob([htmlFormFile], { type: htmlFormFile.type }),
      htmlFormFile.name || 'file'
    );
  ...
  return dispatch => {
    ...
    return fetchWithAuth(`${API_BASE_URI}/ideas/attach-content`, config)
      .then(response => response.json().then(body => ({ body, response })))
      .then(({ body, response }) => {
        if (!response.ok) {
          dispatch(
            uploadFileContentError(`Failed to upload file. ${body.error}`)
          );
          return Promise.reject('Failed to upload file');
        }
...
export const UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_REQUEST =
  'UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_REQUEST';
export const UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_SUCCESS =
  'UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_SUCCESS';
export const UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_FAILURE =
  'UPLOAD_FILE_ON_NEW_COMMENT_CONTENT_FAILURE';
...
function uploadFileOnNewCommentContentSuccess(
  uploadedFileMeta,
  ideaId,
  oldFile
) {
...
export function uploadFileOnNewCommentContent(
  ideaId,
  persistenceId,
  htmlFormFile
) {
  ...
  return dispatch => {
    ...
    console.log(ideaId);
    console.log(htmlFormFile);
    return fetchWithAuth(`${API_BASE_URI}/ideas/comment/attach-content`, config)
      .then(response => response.json().then(body => ({ body, response })))
      .then(({ body, response }) => {
        if (!response.ok) {
          dispatch(
            uploadOnNewCommentFileContentError(
              `Failed to upload file. ${body.error}`
            )
          );
          return Promise.reject('Failed to upload file');
        }
        ...
        console.log(body);
        dispatch(
          uploadFileOnNewCommentContentSuccess(body, ideaId, htmlFormFile)
        );
        return true;
      })
      .catch(err => {
        dispatch(
          uploadFileOnNewCommentContentError(`Failed to upload file. ${err}`)
        );
        console.log('Error: ', err);
      });
  };
    ...
    return fetchWithAuth(`${API_BASE_URI}/ideas/comment/attach`, config)
      ...
      .then(({ body, response }) => {
        if (!response.ok) {
          ...
          console.log(response);
          dispatch(
            removeFilesOnNewCommentError(`Failed to upload file. ${body.error}`)
          );
          return Promise.reject('Failed to remove file');
        }
...
function uploadFileSuccess(dispatch, uploadedFileMeta, ideaId, htmlFormFile) {
  ...
  dispatch(
    uploadFileContent(ideaId, uploadedFileMeta.persistenceId, htmlFormFile)
  );
  return {
...
export const UPLOAD_FILE_ON_NEW_COMMENT_REQUEST =
  'UPLOAD_FILE_ON_NEW_COMMENT_REQUEST';
export const UPLOAD_FILE_ON_NEW_COMMENT_SUCCESS =
  'UPLOAD_FILE_ON_NEW_COMMENT_SUCCESS';
export const UPLOAD_FILE_ON_NEW_COMMENT_FAILURE =
  'UPLOAD_FILE_ON_NEW_COMMENT_FAILURE';
...
function uploadFileOnNewCommentSuccess(
  dispatch,
  uploadedFileMeta,
  ideaId,
  htmlFormFile
  ...
  dispatch(
    uploadFileOnNewCommentContent(
      ideaId,
      uploadedFileMeta.persistenceId,
      htmlFormFile
    )
  );
  return {
...
export const REMOVE_FILES_ON_NEW_COMMENT_REQUEST =
  'REMOVE_FILES_ON_NEW_COMMENT_REQUEST';
export const REMOVE_FILES_ON_NEW_COMMENT_SUCCESS =
  'REMOVE_FILES_ON_NEW_COMMENT_SUCCESS';
export const REMOVE_FILES_ON_NEW_COMMENT_FAILURE =
  'REMOVE_FILES_ON_NEW_COMMENT_FAILURE';
